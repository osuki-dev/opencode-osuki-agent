...
test("package exports the built plugin and retains its workflow assets", async () => {
  ...
  const staging = await mkdtemp(join(tmpdir(), "osuki package "))
  try {
    for (const name of ["src", "agents", "skills", "package.json"])
    ...
    const entry = Bun.pathToFileURL(join(staging, manifest.exports["."]))
